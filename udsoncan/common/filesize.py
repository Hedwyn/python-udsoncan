...
class Filesize:
        ...
        if width is not None:
            ...
            if compressed is not None:
                if compressed > maxsize:
                    raise ValueError(
                        "With width=%d, compressed size must be smaller than %d"
                        % (width, maxsize)
                    )
            ...
            if uncompressed is not None:
                if uncompressed > maxsize:
                    raise ValueError(
                        "With width=%d, uncompressed size must be smaller than %d"
                        % (width, maxsize)
                    )
        else:
            width_value = 0
    ...
    def __str__(self) -> str:
        uncompressed_str = (
            "None" if self.uncompressed is None else "0x%02x" % self.uncompressed
        )
        compressed_str = (
            "None" if self.compressed is None else "0x%02x" % self.compressed
        )
        width_str = "None" if self.width is None else "0x%02x" % self.width

        return "Filesize<Uncompressed=%s, Compressed=%s. Width=%s>" % (
            uncompressed_str,
            compressed_str,
            width_str,
        )
    ...
    def __repr__(self) -> str:
        ...
        return "<Filesize: Uncompressed=%s, Compressed=%s. Width=%s at 0x%08x>" % (
            uncompressed_str,
            compressed_str,
            width_str,
            id(self),
