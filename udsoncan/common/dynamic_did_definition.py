...
class DynamicDidDefinition:
    ...
    def add(self, *args, **kwargs) -> None:
        ...
        entry: Optional[
            Union[
                "DynamicDidDefinition.ByMemloc", "DynamicDidDefinition.ByDidDefinition"
            ]
        ] = None
        ...
        if self.definition_type is None:
            ...
            if entry.__class__ != self.definition_type:
                raise ValueError(
                    "It is not possible to define a composite dynamic DID by memory address and by source DID at the same time."
                )
    ...
    def get_alfid(self) -> AddressAndLengthFormatIdentifier:
        if not self.is_by_memory_address():
            raise ValueError(
                "No AddressAndLengthFormatIdentifier available in this DynamicDidDefinition. Only available when defined by memory address"
            )
        ...
        if alfid is None:
            raise ValueError(
                "No AddressAndLengthFormatIdentifier avaialble"
            )  # should never happen.
