...
class AddressAndLengthFormatIdentifier:
    ...
    address_map = {8: 1, 16: 2, 24: 3, 32: 4, 40: 5, 48: 6, 56: 7, 64: 8}

    memsize_map = {8: 1, 16: 2, 24: 3, 32: 4, 40: 5, 48: 6, 56: 7, 64: 8}
    ...
    def __init__(self, address_format: int, memorysize_format: int):
        if address_format not in self.address_map:
            raise ValueError(
                "address_format must ba an integer selected from : %s "
                % (self.address_map.keys())
            )

        if not isinstance(memorysize_format, int) or not isinstance(
            address_format, int
        ):
        ...
        if memorysize_format not in self.memsize_map:
            raise ValueError(
                "memorysize_format must be an integer selected from : %s"
                % (self.memsize_map.keys())
            )
    ...
    def get_byte_as_int(self) -> int:
        return (
            (self.memsize_map[self.memorysize_format] << 4)
            | (self.address_map[self.address_format])
        ) & 0xFF
