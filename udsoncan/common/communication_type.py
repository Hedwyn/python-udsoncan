...
class CommunicationType:
    ...
    def __init__(
        self,
        subnet: Union[Subnet, int],
        normal_msg: bool = False,
        network_management_msg: bool = False,
    ):
        if not isinstance(subnet, self.Subnet):
            subnet = self.Subnet(subnet)

        if not isinstance(normal_msg, bool) or not isinstance(
            network_management_msg, bool
        ):
            raise ValueError(
                "message type (normal_msg, network_management_msg) must be valid boolean values"
            )
