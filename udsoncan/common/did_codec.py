...
class DidCodec:
    ...
    def encode(self, *did_value: Any) -> bytes:
        if self.packstr is None:
            raise NotImplementedError(
                'Cannot encode DID to binary payload. Codec has no "encode" implementation'
            )
    ...
    def decode(self, did_payload: bytes) -> Any:
        if self.packstr is None:
            raise NotImplementedError(
                'Cannot decode DID from binary payload. Codec has no "decode" implementation'
            )
    ...
    def __len__(self) -> int:
        if self.packstr is None:
            raise NotImplementedError(
                'Cannot tell the payload size. Codec has no "__len__" implementation'
            )
...
class AsciiCodec(DidCodec):
    ...
    def decode(self, string_bin: bytes) -> Any:
        ...
        if len(string_ascii) != self.string_len:
            raise ValueError(
                "Trying to decode a string of %d bytes but codec expects %d bytes"
                % (len(string_ascii), self.string_len)
            )
        return string_ascii
