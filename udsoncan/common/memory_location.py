...
from udsoncan.common import AddressAndLengthFormatIdentifier
...
class MemoryLocation:
    ...
    def __init__(
        ...
        address: int,
        memorysize: int,
        address_format: Optional[int] = None,
        memorysize_format: Optional[int] = None,
    ):
        ...
        self.alfid = AddressAndLengthFormatIdentifier(
            memorysize_format=memorysize_format, address_format=address_format
        )

    # This is used by the client/server to set a format from a config object while letting the user override it
    def set_format_if_none(
        self,
        address_format: Optional[int] = None,
        memorysize_format: Optional[int] = None,
    ) -> None:
        ...
        try:
            ...
            address_format = (
                ...
                else self.autosize_address(self.address)
            )
            memorysize_format = (
                self.memorysize_format
                if self.memorysize_format is not None
                else self.autosize_memorysize(self.memorysize)
            )
    ...
    @classmethod
    def from_bytes(
        cls, address_bytes: bytes, memorysize_bytes: bytes
    ) -> "MemoryLocation":
        ...
        memorysize_bytes_padded = (
            b"\x00" * (8 - len(memorysize_bytes)) + memorysize_bytes
        )
        ...
        return cls(
            ...
            memorysize_format=memorysize_format,
        )

    def __str__(self) -> str:
        return "Address=0x%x (%d bits), Size=0x%x (%d bits)" % (
            self.address,
            self.alfid.address_format,
            self.memorysize,
            self.alfid.memorysize_format,
        )
