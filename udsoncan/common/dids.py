...
from .did_codec import DidCodec
...
class DataIdentifier:
    ...
    def name_from_id(cls, did: int) -> Optional[str]:
        ...
        if not isinstance(did, int) or did < 0 or did > 0xFFFF:
            raise ValueError(
                "Data IDentifier must be a valid integer between 0 and 0xFFFF"
            )
...
def check_did_config(
    didlist: Union[int, List[int]], didconfig: Optional[Dict]
) -> DIDConfig:
    ...
    for did in didlist:
        if did not in didconfig:
            if "default" not in didconfig:
                raise ConfigError(
                    did,
                    msg="Actual data identifier configuration contains no definition for data identifier 0x%04x"
                    % did,
                )
...
def fetch_codec_definition_from_config(
    did: int, didconfig: DIDConfig
) -> CodecDefinition:
...
def make_did_codec_from_definition(
    didconfig: Union[CodecDefinition, IOConfigEntry],
) -> DidCodec:
    if isinstance(
        didconfig, DidCodec
    ):  # the given object already is a DidCodec instance
        return didconfig
    ...
    if isinstance(didconfig, str):
        if len(didconfig) == 0:
            raise ValueError(
                "pack/unpack string given for Codec config should not be empty."
            )
        return DidCodec(packstr=didconfig)

    raise ValueError(
        "Given codec of type %s is not a valid DidCodec" % (type(didconfig))
    )
