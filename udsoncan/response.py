...
class Response:
    ...
    def get_payload(self) -> bytes:
        ...
        if self.service is None:
            raise ValueError(
                "Cannot make payload from response object. Service is not set"
            )

        if not isinstance(self.service, BaseService) and not issubclass(
            self.service, BaseService
        ):
            raise ValueError(
                "Cannot make payload from response object. Given service is not a valid service object"
            )

        if not isinstance(self.code, int):
            raise ValueError(
                "Cannot make payload from response object. Given response code is not a valid integer"
            )
        ...
        if payload[0] != 0x7F:  # Positive
            ...
            if response.service is None:
                ...
                response.invalid_reason = (
                    "Payload first byte is not a know service response ID."
                )
                return response
            ...
            if len(payload) < 2 and response.service.has_response_data():
                ...
                response.invalid_reason = (
                    "Payload must be at least 2 bytes long (service and response)"
                )
                return response
            ...
            if len(payload) < 2:
                ...
                return response
            response.service = BaseService.from_request_id(
                payload[1]
            )  # Request id, not response id

            if response.service is None:
                response.valid = False
                response.invalid_reason = (
                    "Payload second byte is not a known service request ID."
                )
                return response
    ...
    def __repr__(self) -> str:
        responsename = (
            Response.Code.get_name(Response.Code.PositiveResponse)
            if self.positive
            else "NegativeResponse(%s)" % self.code_name
        )
        bytesize = len(self.data) if self.data is not None else 0
        service_name = "NoService"
        if self.service is not None:
            service_name = self.service.get_name()
        return "<%s: [%s] - %d data bytes at 0x%08x>" % (
            responsename,
            service_name,
            bytesize,
            id(self),
