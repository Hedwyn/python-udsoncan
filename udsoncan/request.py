...
class Request:
    ...
    def __init__(
        ...
        if self.service is not None:
            if suppress_positive_response and self.service.use_subfunction() == False:
                raise ValueError(
                    "Cannot suppress positive response for service %s. This service does not have a subfunction"
    ...
    def get_payload(self, suppress_positive_response: Optional[bool] = None) -> bytes:
        ...
        if not issubclass(self.service, BaseService):
            raise ValueError(
                "Cannot generate a payload. Given service is not a subclass of BaseService"
            )
        use_subfunction = self.service.use_subfunction()
        if use_subfunction and not isinstance(self.subfunction, int):
            raise ValueError(
                "Cannot generate a payload. Given subfunction is not a valid integer"
            )

        requestid = (
            self.service.request_id()
        )  # Returns the service ID used to make a client request
        ...
        if use_subfunction:
            ...
            if (
                ...
                or self.suppress_positive_response == True
            ):
                raise ValueError(
                    "Cannot suppress positive response for service %s. This service does not have a subfunction"
                    % (self.service.get_name())
            ...
            if req.service is not None:  # Invalid service ID will make service None
                ...
                if req.service.use_subfunction():
                    ...
                    if len(payload) >= offset + 1:
                        ...
                        req.suppress_positive_response = (
                            True if payload[1] & 0x80 > 0 else False
                        )
                if len(payload) > offset + 1:
