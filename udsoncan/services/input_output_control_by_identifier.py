...
class InputOutputControlByIdentifier(BaseService):
    ...
    class ResponseData(BaseResponseData):
        ...
        def __init__(
            self,
            did_echo: int,
            control_param_echo: Optional[int] = None,
    ...
    def make_request(
        ...
        if masks is not None:
            ...
            if not isinstance(masks, IOMasks) and not isinstance(masks, bool):
                raise ValueError(
                    "masks must be an instance of IOMask or a boolean value"
                )
        ...
        # tools.
        codec = make_did_codec_from_definition(
            io_config_entry
        )  # Get IO codec from config
        ...
        if masks is not None:  # Skip the masks byte if none is given.
            if isinstance(masks, bool):
                ...
                if (
                    "mask_size" in io_config_entry
                    and io_config_entry["mask_size"] is not None
                ):
                    ...
                    raise ConfigError(
                        "mask_size",
                        msg="Given mask is boolean value, indicating that all mask should be set to same value, but no mask_size is defined in configuration. Cannot guess how many bits to set.",
                    )
                ...
                if "mask" not in io_config_entry:
                    raise ConfigError(
                        "mask",
                        msg="Cannot apply given mask. Input/Output configuration does not define their position (and size).",
                    )
                ...
                for mask_name in given_masks:
                    if mask_name not in masks_config:
                        raise ConfigError(
                            "mask_size",
                            msg="Cannot set mask bit for mask %s. The configuration does not define its position"
                            % (mask_name),
                        )
    ...
    def interpret_response(
        ...
        min_response_size = (
            2 if control_param is not None else 1
        )  # Spec specifies that if first byte is a ControlParameter, it must be echoed back by the server

        if len(response.data) < min_response_size:
            raise InvalidResponseException(
                response, "Response must be at least %d bytes long" % min_response_size
            )
        ...
        ioconfig_validated = tools.check_io_config(
            did, ioconfig
        )  # IO DIDs are defined in client config.
        io_config_entry = tools.fetch_io_entry_from_config(
            did, ioconfig_validated
        )  # Get requested did definition (given or default)
        codec = make_did_codec_from_definition(
            io_config_entry
        )  # Get IO codec from config
        ...
        if control_param is not None:
            if len(response.data) < next_byte:
                raise InvalidResponseException(
                    response,
                    "Response should include an echo of the InputOutputControlParameter (0x%02x)"
                    % control_param,
                )
        ...
        if len(response.data) >= next_byte:
            ...
            if len(remaining_data) > payload_size:
                if remaining_data[payload_size:] == b"\x00" * (
                    len(remaining_data) - payload_size
                ):
            ...
            try:
                ...
                raise InvalidResponseException(
                    response,
                    "Response from server could not be decoded. Exception is : %s" % e,
                )
