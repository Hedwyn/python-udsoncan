...
from udsoncan.base_service import BaseService, BaseResponseData, BaseSubfunction
...
class Authentication(BaseService):
    ...
    def make_request(
        ...
        if authentication_task in (
            ...
            if authentication_task in (
                ...
                # certificateClient
                data = Authentication._append_byes_parameter(
                    data, certificate_client, "Certificate Client"
                )

                # challengeClient
                data = Authentication._append_byes_parameter(
                    data, challenge_client, "Challenge Client"
                )
            else:
                if (
                    not isinstance(algorithm_indicator, bytes)
                    or len(algorithm_indicator) != 16
                ):
                    raise ValueError(
                        f"{algorithm_indicator!r} must be a bytes object of length 16"
                    )
                # algorithmIndicator
                data += algorithm_indicator
            ...
            data = Authentication._append_byes_parameter(
                data, proof_of_ownership_client, "Proof Of Ownership Client"
            )
            # ephemeralPublicKeyClient
            data = Authentication._append_byes_parameter(
                data, ephemeral_public_key_client, "Ephemeral Public Key" " Client"
            )
            ...
            tools.validate_int(
                ...
                min=0,
                max=0xFFFF,
                name="Certificate Evaluation ID",
            )
            # certificateEvaluationId
            data = struct.pack(">H", certificate_evaluation_id)
            # certificateData
            data = Authentication._append_byes_parameter(
                data, certificate_data, "Certificate Data"
            )
            ...
            # additionalParameter
            data = Authentication._append_byes_parameter(
                data, additional_parameter, "Additional Parameter"
            )
    ...
    def interpret_response(cls, response: Response) -> InterpretedResponse:
        ...
        if len(response.data) < 2:
            raise InvalidResponseException(
                response, "Response data must be at least 2 bytes"
            )
        ...
        if subfunction in (
            ...
            challenge_server = Authentication._extract_byes_parameter(
                response, response.data[offset:], "Challenge Server"
            )
            offset += 2 + len(challenge_server)

            ephemeral_public_key_server = Authentication._extract_byes_parameter(
                response, response.data[offset:], "Ephemeral Public Key Server"
            )
            ...
            certificate_server = Authentication._extract_byes_parameter(
                response, response.data[offset:], "Certificate Server"
            )
            offset += 2 + len(certificate_server)

            proof_of_ownership_server = Authentication._extract_byes_parameter(
                response, response.data[offset:], "Proof Of Ownership Server"
            )
            ...
            session_key_info = Authentication._extract_byes_parameter(
                response, response.data[offset:], "Session Key Info"
            )
            ...
            if (
                len(response.data[offset:]) < 16
            ):  # algorithmIndicator is exactly 16 bytes
                raise InvalidResponseException(
                    response,
                    f"Not enough data for Algorithm Indicator (16 bytes), reset of"
            ...
            if (
                ...
                needed_additional_parameter = Authentication._extract_byes_parameter(
                    response, response.data[offset:], "Needed Additional Parameter"
                )
        ...
        if len(response.data) > offset:
            raise InvalidResponseException(
                response, "Response data had more bytes than expected"
            )
