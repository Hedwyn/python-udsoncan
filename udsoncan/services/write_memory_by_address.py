...
class WriteMemoryByAddress(BaseService):
    ...
    def interpret_response(
        cls, response: Response, memory_location: MemoryLocation
    ) -> InterpretedResponse:
        """
        ...
        if len(response.data) < expected_response_size:
            raise InvalidResponseException(
                response,
                "Repsonse should be at least %d bytes" % (expected_response_size),
            )
        ...
        response.service_data = cls.ResponseData(
            ...
            memory_location_echo=MemoryLocation.from_bytes(
