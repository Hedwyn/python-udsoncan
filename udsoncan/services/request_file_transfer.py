...
class RequestFileTransfer(BaseService):
    ...
    class ModeOfOperation(
        BaseSubfunction
    ):  # Not really a subfunction, but we wantto inherit the helpers in BaseSubfunction class
        """
    ...
    @classmethod
    def normalize_data_format_identifier(
        cls, dfi: Optional[DataFormatIdentifier]
    ...
    def make_request(
        ...
        if len(path_ascii) > 0xFFFF:
            raise ValueError(
                "Path length must be smaller or equal than 65535  bytes (16 bits) when encoded in ASCII"
            )
        ...
        if use_dfi:
            ...
            if dfi is not None:
                raise ValueError(
                    "DataFormatIdentifier is not needed with ModeOfOperation=%d" % moop
                )
        ...
        if use_filesize:
            ...
            if filesize.compressed is None:
                filesize = Filesize(
                    uncompressed=filesize.uncompressed,
    ...
    def interpret_response(
        cls, response: Response, tolerate_zero_padding: bool = True
    ) -> InterpretedResponse:
        """
        ...
        response.service_data = cls.ResponseData(moop_echo=int(response.data[0]))
        ...
        has_fileposition = response.service_data.moop_echo in [
            cls.ModeOfOperation.ResumeFile
        ]
        ...
        if has_dfi:
            if len(response.data) < cursor + 1:
                raise InvalidResponseException(
                    response, "Missing DataFormatIdentifier in received response"
                )
            ...
            if (
                response.service_data.moop_echo == cls.ModeOfOperation.ReadDir
                and dfi_int != 0
            ):
                raise InvalidResponseException(
                    response,
        ...
        if (
            has_uncompressed_filesize
            and response.service_data.moop_echo == cls.ModeOfOperation.ReadDir
        ):
        ...
        if has_fileposition:
            ...
            if len(response.data) < cursor + fposl:
                raise InvalidResponseException(
                    response, "Missing or incomplete FilePosition in received response"
                )
        ...
        if len(response.data) > cursor:
            if (
                response.data[cursor:] == b"\x00" * (len(response.data) - cursor)
                and tolerate_zero_padding
            ):
                pass
            else:
                raise InvalidResponseException(
                    response, "Response payload has extra data that has no meaning"
                )
