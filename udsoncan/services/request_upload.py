...
class RequestUpload(BaseService):
    ...
    class InterpretedResponse(Response):
        service_data: "RequestUpload.ResponseData"
    ...
    @classmethod
    def make_request(
        cls, memory_location: MemoryLocation, dfi: Optional[DataFormatIdentifier] = None
    ) -> Request:
        """
        ...
        request.data += (
            memory_location.alfid.get_byte()
    ...
    def interpret_response(cls, response: Response) -> InterpretedResponse:
        ...
        if lfid > 8:
            raise NotImplementedError(
                "This client does not support number bigger than %d bits" % (8 * 8)
            )
        ...
        return cast(RequestUpload.InterpretedResponse, response)
