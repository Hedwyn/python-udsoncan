...
class SecurityAccess(BaseService):
    ...
    def make_request(cls, level: int, mode: int, data=bytes()) -> Request:
        ...
        req = Request(
            service=cls, subfunction=cls.normalize_level(mode=mode, level=level)
        )
    ...
    def interpret_response(cls, response: Response, mode: int) -> InterpretedResponse:
        ...
        if len(response.data) < minlength:
            raise InvalidResponseException(
                response, "Response data must be at least %d bytes" % (minlength)
            )

        response.service_data = cls.ResponseData(security_level_echo=response.data[0])
    ...
    def validate_mode(cls, mode: int):
        if mode not in (cls.Mode.RequestSeed, cls.Mode.SendKey):
            raise ValueError(
                "Given mode must be either be RequestSeed (%d) or SendKey (%d)."
                % (cls.Mode.RequestSeed, cls.Mode.SendKey)
            )
