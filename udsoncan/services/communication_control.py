...
class CommunicationControl(BaseService):
    ...
    def normalize_communication_type(
        self, communication_type: Union[int, bytes, CommunicationType]
    ) -> CommunicationType:
        if (
            not isinstance(communication_type, CommunicationType)
            and not isinstance(communication_type, int)
            and not isinstance(communication_type, bytes)
        ):
            raise ValueError(
                "communication_type must either be a CommunicationType object or an integer"
            )
    ...
    def make_request(
        ...
        if node_id is not None:
            tools.validate_int(
                node_id, min=0, max=0xFFFF, name="nodeIdentificationNumber"
            )
    ...
    def interpret_response(cls, response: Response) -> InterpretedResponse:
        ...
        if len(response.data) < 1:
            raise InvalidResponseException(
                response, "Response data must be at least 1 byte"
            )

        response.service_data = cls.ResponseData(control_type_echo=response.data[0])
