...
class ReadDTCInformation(BaseService):
    ...
    @classmethod
    def assert_severity_mask(
        cls, severity_mask: Optional[int], subfunction: int
    ) -> None:
        if severity_mask is None:
            raise ValueError(
                "severity_mask must be provided for subfunction 0x%02x" % subfunction
            )
    ...
    def assert_status_mask(cls, status_mask: Optional[int], subfunction: int) -> None:
        if status_mask is None:
            raise ValueError(
                "status_mask must be provided for subfunction 0x%02x" % subfunction
            )
        tools.validate_int(status_mask, min=0, max=0xFF, name="Status mask")

    @classmethod
    def assert_memory_selection(
        cls, memory_selection: Optional[int], subfunction: int
    ) -> None:
        if memory_selection is None:
            raise ValueError(
                "memory_selection must be provided for subfunction 0x%02x" % subfunction
            )
    ...
    def assert_dtc(cls, dtc: Optional[int], subfunction: int) -> None:
        if dtc is None:
            raise ValueError(
                "A dtc value must be provided for subfunction 0x%02x" % subfunction
            )
        tools.validate_int(dtc, min=0, max=0xFFFFFF, name="DTC")

    @classmethod
    def assert_snapshot_record_number(
        cls, snapshot_record_number: Optional[int], subfunction: int
    ) -> None:
        if snapshot_record_number is None:
            raise ValueError(
                "snapshot_record_number must be provided for subfunction 0x%02x"
                % subfunction
            )
        tools.validate_int(
            snapshot_record_number, min=0, max=0xFF, name="Snapshot record number"
        )

    @classmethod
    def assert_extended_data_record_number(
        cls,
        extended_data_record_number: Optional[int],
        subfunction: int,
        maxval: int = 0xFF,
    ) -> None:
        if extended_data_record_number is None:
            raise ValueError(
                "extended_data_record_number must be provided for subfunction 0x%02x"
                % subfunction
            )
        tools.validate_int(
            ...
            min=0,
            max=maxval,
            name="Extended data record number",
        )

    @classmethod
    def assert_extended_data_size_int_or_dict(
        cls, extended_data_size: Optional[Union[int, Dict[int, int]]], subfunction: int
    ) -> None:
        if extended_data_size is None:
            raise ValueError(
                "extended_data_size must be provided as length of data is not given by the server for subfunction %d."
                % subfunction
            )
        if isinstance(extended_data_size, int):
            tools.validate_int(
                extended_data_size, min=0, max=0xFFF, name="Extended data size"
            )
        if isinstance(extended_data_size, dict):
            for dtcid in extended_data_size:
                tools.validate_int(
                    extended_data_size[dtcid],
                    min=0,
                    max=0xFFF,
                    name="Extended data size for DTC=0x%06x" % dtcid,
                )

    @classmethod
    def pack_dtc(cls, dtcid: int) -> bytes:
        return struct.pack(
            "BBB", (dtcid >> 16) & 0xFF, (dtcid >> 8) & 0xFF, (dtcid >> 0) & 0xFF
        )

    @classmethod
    def check_subfunction_valid(
        cls, subfunction: int, standard_version: int = latest_standard
    ) -> None:
        ...
        if subfunction in subfunction2020 and standard_version < 2020:
            raise NotImplementedError(
                "The subfunction 0x%02x has been introduced in standard version 2020 but decoding is requested to be done as per %d version"
    ...
    def make_request(
        ...
        if subfunction in request_subfn_no_param:  # Service ID + Subfunction
            ...
            subfunction
            in request_subfn_mask_record_plus_snapshot_record_number_plus_memory_selection
        ):
            cls.assert_dtc(dtc, subfunction)
            cls.assert_snapshot_record_number(snapshot_record_number, subfunction)
            ...
            req.data = cls.pack_dtc(dtc) + struct.pack(
                "BB", snapshot_record_number, memory_selection
            )
            ...
            cls.assert_dtc(dtc, subfunction)
            assert dtc is not None  # mypy nitpick
            cls.assert_extended_data_record_number(
                extended_data_record_number, subfunction
            )
            req.data = cls.pack_dtc(dtc) + struct.pack("B", extended_data_record_number)

        elif (
            subfunction
            in request_subfn_mask_record_plus_extdata_record_number_plus_memory_selection
        ):
            cls.assert_dtc(dtc, subfunction)
            assert dtc is not None  # mypy nitpick
            cls.assert_memory_selection(memory_selection, subfunction)
            ...
            req.data = cls.pack_dtc(dtc) + struct.pack(
                "BB", extended_data_record_number, memory_selection
            )
            ...
            cls.assert_extended_data_record_number(
                extended_data_record_number, subfunction, maxval=0xEF
            )  # Maximum specified by ISO-14229:2020
    ...
    def interpret_response(
        ...
        firstbyte_is_memory_selection_echo = (
            subfunction in subfunctions_with_memory_selection
        )
        ...
        if len(response.data) < 1:
            raise InvalidResponseException(
                response, "Response must be at least 1 byte long (echo of subfunction)"
            )
        ...
        if (
            subfunction
            in response_subfn_dtc_availability_mask_plus_dtc_record
            + response_subfn_dtc_availability_mask_plus_dtc_record_with_severity
        ):
            if subfunction in response_subfn_dtc_availability_mask_plus_dtc_record:
                ...
                subfunction
                in response_subfn_dtc_availability_mask_plus_dtc_record_with_severity
            ):
                dtc_size = (
                    6  # DTC ID (3) + Status (1) + Severity (1) + FunctionalUnit (1)
                )
            ...
            if firstbyte_is_memory_selection_echo:
                if len(response.data) < 3:
                    raise InvalidResponseException(
                        response,
                        "Response must be at least 3 bytes long (echo of subfunction, echo of MemorySelection and DTCStatusAvailabilityMask)",
                    )
                ...
                if len(response.data) < 2:
                    raise InvalidResponseException(
                        response,
                        "Response must be at least 2 byte long (echo of subfunction and DTCStatusAvailabilityMask)",
                    )
                actual_byte = 1

            response.service_data.status_availability = Dtc.Status.from_byte(
                response.data[actual_byte]
            )
            actual_byte += 1
            ...
            while True:  # Loop until we have read all dtcs
                if len(response.data) <= actual_byte:
                    ...
                    if (
                        ...
                        and response.data[actual_byte:] == b"\x00" * partial_dtc_length
                    ):
                        break
                    else:
                        ...
                        if (
                            ...
                            raise InvalidResponseException(
                                response,
                                "Incomplete DTC record. Missing %d bytes to response to complete the record"
        ...
        # The 2 following subfunction responses have different purposes but their constructions are very similar.
            ...
            subfunction
            in response_subfn_dtc_plus_fault_counter
            + response_subfn_dtc_plus_sapshot_record
        ):
            dtc_size = 4
            if len(response.data) < 1:
                raise InvalidResponseException(
                    response,
                    "Response must be at least 1 byte long (echo of subfunction)",
                )

            actual_byte = 1  # Increasing index
            dtc_map: Dict[int, Dtc] = (
                dict()
            ...
            while True:  # Loop until we have read all dtcs
                if len(response.data) <= actual_byte:
                    ...
                    if dtc_bytes == b"\x00" * dtc_size and ignore_all_zero_dtc:
                        ...
                        if (
                            dtcid in dtc_map
                            and subfunction in response_subfn_dtc_plus_sapshot_record
                        ):
                            dtc = dtc_map[dtcid]
                        else:
                            dtc = Dtc(dtcid)
        ...
        # This group of responses returns a number of DTCs available
            ...
            if len(response.data) < 5:
                raise InvalidResponseException(
                    response, "Response must be exactly 5 bytes long "
                )
        ...
        # <DTC,RecordNumber1,NumberOfDid_X,DID1,DID2,...DIDX, RecordNumber2,NumberOfDid_Y,DID1,DID2,...DIDY, etc>
            ...
            if len(response.data) < minlength:
                raise InvalidResponseException(
                    response, "Response must be at least %d bytes long" % minlength
                )
            ...
            dtc = Dtc(
                struct.unpack(
                    ">L", b"\x00" + response.data[actual_byte : (actual_byte + 3)]
                )[0]
            )
            ...
            tools.validate_int(
                dtc_snapshot_did_size, min=1, max=8, name="dtc_snapshot_did_size"
            )
            ...
            while True:  # Loop until we have read all dtcs
                ...
                if tolerate_zero_padding and remaining_data == b"\x00" * len(
                    remaining_data
                ):
                    break
                ...
                if len(remaining_data) < 2:
                    raise InvalidResponseException(
                        response,
                        'Incomplete response from server. Missing "number of identifier" and following data',
                    )
                ...
                if number_of_did == 0:
                    raise InvalidResponseException(
                        response,
                        "Server returned snapshot record #%d with no data identifier included"
                        % (record_number),
                    )
                ...
                if len(remaining_data) < 2 + dtc_snapshot_did_size:
                    raise InvalidResponseException(
                        response,
                        "Incomplete response from server. Missing DID number and associated data.",
                    )
                ...
                for i in range(number_of_did):
                    ...
                    codec_definition = fetch_codec_definition_from_config(
                        did, didconfig
                    )
                    ...
                    if len(remaining_data[data_offset:]) < len(codec):
                        raise InvalidResponseException(
                            response,
                            "Incomplete response. Data for DID 0x%04x is only %d bytes while %d bytes is expected"
        ...
        # <RecordNumber1, DTC1,NumberOfDid_X,DID1,DID2,...DIDX, RecordNumber2,DTC2, NumberOfDid_Y,DID1,DID2,...DIDY, etc>
            ...
            if len(response.data) < 2:
                raise InvalidResponseException(
                    response,
                    "Response must be at least 2 bytes long. Subfunction echo + RecordNumber ",
                )
            ...
            while True:  # Loop through response data
                ...
                if (
                    remaining_data == b"\x00" * len(remaining_data)
                    and tolerate_zero_padding
                ):
                    break

                # If record number received but no DTC provided (allowed according to standard), we exit.
                if (
                    len(remaining_data) == 1
                    or tolerate_zero_padding
                    and remaining_data[1:] == b"\x00" * len(remaining_data[1:])
                ):
                    break

                if (
                    len(remaining_data) < 5
                ):  # Partial DTC (No DTC at all is checked above)
                    raise InvalidResponseException(
                        response,
                        'Incomplete response from server. Missing "DTCAndStatusRecord" and following data',
                    )

                if len(remaining_data) < 6:
                    raise InvalidResponseException(
                        response,
                        "Incomplete response from server. Missing number of data identifier",
                    )
                ...
                if len(remaining_data) < dtc_snapshot_did_size:
                    raise InvalidResponseException(
                        response,
                        "Incomplete response from server. Missing DID and associated data",
                    )
        ...
        # These subfunctions include DTC ExtraData. We give it raw to user.
            ...
            if len(response.data) < minlength:
                missing_data = (
                    ...
                    if firstbyte_is_memory_selection_echo
                    else "DTCAndStatusRecord"
                )
                raise InvalidResponseException(
                    response,
                    "Incomplete response from server. Missing %s" % missing_data,
                )
            ...
            while actual_byte < len(response.data):  # Loop through data
                ...
                if record_number == 0:
                    if (
                        ...
                        raise InvalidResponseException(
                            response,
                            "Extended data record number given by the server is 0 but this value is a reserved value.",
                        )
                ...
                if len(remaining_data) < size:
                    raise InvalidResponseException(
                        response,
                        "Incomplete response from server. Length of extended data for DTC 0x%06x with record number 0x%02x is %d bytes but smaller than given data_size of %d bytes"
            ...
            if len(response.data) < 2:
                raise InvalidResponseException(
                    response,
                    "Incomplete response from server. Missing DTCExtDataRecordNumber",
                )
            ...
            if record_number > 0xEF:
                raise InvalidResponseException(
                    response,
                    "Server returned a RecordNumber of %d which is out of range (00-EF) according to ISO-14229:2020",
                    record_number,
                )
            ...
            while True:
                ...
                remaining_data = response.data[actual_byte:]
                all_zero_ending = (
                    True if remaining_data == b"\x00" * len(remaining_data) else False
                )

                if all_zero_ending:
                    try:
                        zero_dtc_size = cls.get_extended_data_size(
                            Dtc(0), extended_data_size
                        )
                    except:
                        zero_dtc_size = None

                    all_zero_dtc_to_read = (
                        zero_dtc_size is not None
                        and len(remaining_data) >= zero_dtc_size + 4
                    ) and ~ignore_all_zero_dtc
                    ...
                    if ~all_zero_dtc_to_read:
                        if tolerate_zero_padding:
                            ...
                            raise InvalidResponseException(
                                response,
                                "Server response ends with a sequence of zero and zero adding is not tolerated",
                            )
                ...
                if len(response.data) - actual_byte < 4:
                    raise InvalidResponseException(
                        response,
                        "Incomplete DTCAndStatusRecord at position %d" % actual_byte,
                    )
                ...
                if dtc.id in received_dtcs:
                    raise InvalidResponseException(
                        response,
                        "The server returned two set of data for the same DTC (0x%06X) and same record number (%d)",
                        (dtc.id, record_number),
                    )
                ...
                if len(response.data) - actual_byte < size:
                    raise InvalidResponseException(
                        response,
                        "Incomplete ExtendedData for DTC 0x%06X. Configuration says that there should be %d bytes of data but server only gave %d bytes."
    ...
    @classmethod
    def get_extended_data_size(
        cls, dtc: Dtc, given_size: Union[int, Dict[int, int]]
    ) -> int:
        size = None
        if isinstance(given_size, int):
            ...
            if dtc.id not in given_size:
                raise ConfigError(
                    key=dtc.id,
                    msg="Server returned extended data for DTC 0x%06X but no data size was specified for this DTC"
                    % (dtc.id),
                )
