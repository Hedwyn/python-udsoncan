...
class ReadDataByIdentifier(BaseService):
    ...
    @classmethod
    def make_request(
        cls, didlist: Union[int, List[int]], didconfig: Optional[DIDConfig]
    ) -> Request:
        """
    ...
    def interpret_response(
        ...
        response.service_data = cls.ResponseData(values={})
        ...
        while True:
            ...
            if len(response.data) <= offset + 1:
                if (
                    tolerate_zero_padding and response.data[-1] == 0
                ):  # One extra byte, but it's a 0 and we accept that. So we're done
                    break
                raise InvalidResponseException(
                    response, "Response given by server is incomplete."
                )

            did = struct.unpack(">H", response.data[offset : offset + 2])[
                0
            ]  # Get the DID number
            if (
                did == 0 and did not in didconfig_validated and tolerate_zero_padding
