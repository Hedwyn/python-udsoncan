...
class DynamicallyDefineDataIdentifier(BaseService):
    ...
    def make_request(
        ...
        diddef: Optional[DynamicDidDefinition] = None,
    ) -> Request:
        """
        ...
        if subfunction in [
            ...
            if did is None:
                raise ValueError(
                    "A DID number must be given with subfunction %d" % (subfunction)
                )
        ...
        if subfunction == cls.Subfunction.defineByIdentifier:
            ...
            diddef_entries = cast(
                List[DynamicDidDefinition.ByDidDefinition], diddef_entries
            )
            for entry_by_did in diddef_entries:
                req.data += struct.pack(
                    ">HBB",
                    entry_by_did.source_did,
                    entry_by_did.position,
                    entry_by_did.memorysize,
                )
    ...
    def interpret_response(cls, response: Response) -> InterpretedResponse:
        ...
        response.service_data = cls.ResponseData(subfunction_echo=int(response.data[0]))

        if response.service_data.subfunction_echo in [
            cls.Subfunction.defineByIdentifier,
            cls.Subfunction.defineByMemoryAddress,
        ]:
            if len(response.data) < 3:
                raise InvalidResponseException(
                    response, "Missing or incomplete DID echo in response"
                )
