...
class TestMemoryLocation(UdsTest):
    def test_memloc1(self):
        memloc = MemoryLocation(
            address=0x1234, memorysize=0x78, address_format=16, memorysize_format=8
        )
    ...
    def test_memloc_override(self):
        ...
        memloc = MemoryLocation(address=0x1234, memorysize=0x78)
        memloc.set_format_if_none(
            address_format=32, memorysize_format=24
        )  # Both at same time.
        self.assertEqual(memloc.get_address_bytes(), b"\x00\x00\x12\x34")
        self.assertEqual(memloc.get_memorysize_bytes(), b"\x00\x00\x78")

    def test_memloc_from_bytes(self):
        memloc = MemoryLocation.from_bytes(
            address_bytes=b"\x12\x34", memorysize_bytes=b"\xff"
        )
        ...
        memloc = MemoryLocation.from_bytes(
            address_bytes=b"\x12\x34\x56", memorysize_bytes=b"\x66\x77\x88"
        )
    ...
    def test_memloc_max_size(self):
        MemoryLocation.from_bytes(
            address_bytes=b"\x12\x34\x56\x78\x9a", memorysize_bytes=b"\xff"
        )
        with self.assertRaises(ValueError):
            MemoryLocation.from_bytes(
                address_bytes=b"\x12\x34\x56\x78\x9a\xbc", memorysize_bytes=b"\xff"
            )

        MemoryLocation.from_bytes(
            address_bytes=b"\x12\x34", memorysize_bytes=b"\x12\x34\x56\x78"
        )
        with self.assertRaises(ValueError):
            MemoryLocation.from_bytes(
                address_bytes=b"\x12\x34", memorysize_bytes=b"\x12\x34\x56\x78\x9a"
            )
...
class TestCommunicationType(UdsTest):
    def test_make(self):
        comtype = CommunicationType(
            subnet=CommunicationType.Subnet.node,
            normal_msg=True,
        ...
        comtype = CommunicationType(
            subnet=CommunicationType.Subnet.network,
            normal_msg=True,
            network_management_msg=False,
        )
        ...
        comtype = CommunicationType(
            subnet=3, normal_msg=True, network_management_msg=True
        )
...
class TestIOMasks(UdsTest):
    ...
    def test_make_dict(self):
        ...
        self.assertEqual(
            m.get_dict(), {"aaa": True, "bbb": True, "ccc": True, "ddd": False}
        )
...
class TestFilesize(UdsTest):
    ...
    def test_width(self):
        ...
        self.assertEqual(
            Filesize(uncompressed=0xFFFFFF, compressed=0xFFFFFFFF).get_width(), 4
        )

        self.assertEqual(
            Filesize(uncompressed=0xFF, compressed=0xFF, width=4).get_width(), 4
        )
        self.assertEqual(
            Filesize(uncompressed=0xFF, compressed=0xFF, width=8).get_width(), 8
        )
    ...
    def test_bytes(self):
        ...
        self.assertEqual(
            Filesize(compressed=0x12345678).get_compressed_bytes(), b"\x12\x34\x56\x78"
        )
        self.assertEqual(
...
class TestDynamicDidDefinition(UdsTest):
    ...
    def test_get_alfid(self):
        ...
        diddef.add(
            MemoryLocation(
                address=0x1234, memorysize=1, address_format=16, memorysize_format=8
            )
        )
        diddef.add(
            MemoryLocation(
                address=0x1235, memorysize=1, address_format=16, memorysize_format=8
            )
        )
        ...
        diddef.add(
            MemoryLocation(
                address=0x1235, memorysize=1, address_format=16, memorysize_format=16
            )
        )
        with self.assertRaises(ValueError):
            alfid = diddef.get_alfid()
