...
class TestIsoTPSocketConnection(UdsTest):
    ...
    def test_open(self):
        ...
        conn = IsoTPSocketConnection(
            interface="vcan0", address=addr, tpsock=self.tpsock1, name="unittest"
        )
    ...
    def test_transmit(self):
        addr1 = isotp.Address(
            isotp.AddressingMode.Normal_11bits, rxid=0x100, txid=0x101
        )
        addr2 = isotp.Address(
            isotp.AddressingMode.Normal_11bits, rxid=0x101, txid=0x100
        )
        conn1 = IsoTPSocketConnection(
            interface="vcan0", address=addr1, tpsock=self.tpsock1, name="unittest"
        )
        conn2 = IsoTPSocketConnection(
            interface="vcan0", address=addr2, tpsock=self.tpsock2, name="unittest"
        )
...
@unittest.skipIf(
    _STACK_POSSIBLE == False,
    "Cannot test TestPythonIsoTpConnection. %s" % _STACK_UNVAILABLE_REASON,
)
class TestPythonIsoTpConnection(UdsTest):
    ...
    def setUp(self):
        ...
        self.conn = PythonIsoTpConnection(
            isotp.NotifierBasedCanStack(
                bus=self.vcan0_bus, notifier=self.notifier, address=addr
            ),
            name="unittest",
        )
        self.conn.open()
    ...
    def test_receive(self):
        self.vcan0_bus.send(
            can.Message(
                arbitration_id=self.stack_rxid,
