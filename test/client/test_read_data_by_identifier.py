...
class TestReadDataByIdentifier(ClientServerTest):
    ...
    def _test_peek_rdbi_single_success(self):
        response = self.udsclient.test_data_identifier(
            0x1122
        )  # not in config, but this is OK
        self.assertTrue(response.positive)
    ...
    def _test_rdbi_single_success_default_did(self):
        ...
        self.assertEqual(values[1], (0x1234,))
        self.assertEqual(
            original_config, self.udsclient.config
    ...
    def test_peek_rdbi_multiple_success(self):
        ...
        self.conn.fromuserqueue.put(
            b"\x62\xaa"
        )  # Positive response, content can be invalid. we don't check with the peek method
    ...
    def test_rdbi_multiple_zero_padding_not_tolerated_exception(self):
        ...
        self.wait_request_and_respond(
            data + b"\x00"
        )  # One extra byte is incomplete DID = invalid response
    ...
    def test_rdbi_variable_size_did(self):
        self.wait_request_and_respond(
            b"\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x05\xaa\xbb\xcc\xdd"
        )
    ...
    def test_rdbi_unknown_did_exception(self):
        self.wait_request_and_respond(
            b"\x62\x00\x09\x12\x34\x00\x02\x56\x78\x00\x03\x11"
        )
    ...
    def test_rdbi_unwanted_did_exception(self):
        self.wait_request_and_respond(
            b"\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x03\x11"
        )
    ...
    def test_rdbi_wrongservice_exception(self):
        self.wait_request_and_respond(
            b"\x50\x00\x01\x12\x34"
        )  # Valid service, but not the one requested
