...
class TestRequestDownload(ClientServerTest):
    ...
    def _test_request_download_success(self):
        memloc = MemoryLocation(
            address=0x1234, memorysize=0xFF, address_format=16, memorysize_format=8
        )
    ...
    def test_request_download_config_format(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x34\x00\x24\x00\x00\x12\x34\x00\xff"
        )  # dfi = 24 and 0 padding
        self.conn.fromuserqueue.put(b"\x74\x20\xab\xcd")  # Positive response

    def _test_request_download_config_format(self):
        self.udsclient.set_configs(
            {"server_address_format": 32, "server_memorysize_format": 16}
        )
    ...
    def test_incomplete_nblock_response_exception(self):
        self.wait_request_and_respond(
            b"\x74\x40\xab\xcd"
        )  # Missing 2 bytes to complete number of block
