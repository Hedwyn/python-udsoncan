...
class TestRequestSeed(ClientServerTest):
    ...
    def test_request_seed_bad_subfn_exception(self):
        self.wait_request_and_respond(
            b"\x67\x06\x99\x88\x77\x66"
        )  # Positive response with wrong subfunction
...
class TestUnlockSecurityService(ClientServerTest):
    ...
    def test_unlock_already_unlocked(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x67\x07\x00\x00\x00\x00"
        )  # Positive response, seed=0 : already unlocked
