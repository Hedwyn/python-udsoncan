...
class TestRequestFileTransfer(ClientServerTest):
    ...
    def test_add_file_success(self):
        ...
        self.assertEqual(
            request,
            b"\x38\x01\x00\x0b"
            + "my_file.txt".encode("ascii")
            + b"\x52\x04\x00\x00\x02\x22\x00\x00\x01\x11",
        )
    ...
    def test_replace_file_extra_bytes_response_zero_padding_ok(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x03\x02\xab\xcd\x52\x00\x00"
        )  # Positive response
    ...
    def test_read_file_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x38\x04\x00\x0b" + "my_file.txt".encode("ascii") + b"\x52"
        )
        self.conn.fromuserqueue.put(
            b"\x78\x04\x02\xab\xcd\x52\x00\x02\x98\x76\x12\x34"
        )  # Positive response

    def _test_read_file_success(self):
        response = self.udsclient.read_file(
            "my_file.txt", dfi=DataFormatIdentifier(compression=5, encryption=2)
        )
        self.assertTrue(response.valid)
        self.assertTrue(response.positive)
    ...
    def test_read_file_extra_bytes_response(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x04\x02\xab\xcd\x52\x00\x02\x98\x76\x12\x34\xaa"
        )  # Positive response
    ...
    def test_read_file_extra_bytes_response_zero_padding_ok(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x04\x02\xab\xcd\x52\x00\x02\x98\x76\x12\x34\x00"
        )  # Positive response
    ...
    def test_read_dir_success(self):
        ...
        self.conn.fromuserqueue.put(
            b"\x78\x05\x02\xab\xcd\x00\x00\x02\x12\x34"
        )  # Positive response
    ...
    def test_read_dir_bad_moop_echo(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x01\x02\xab\xcd\x00\x00\x02\x12\x34"
        )  # Positive response
    ...
    def test_read_dir_dfi_not_zero(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x05\x02\xab\xcd\x52\x00\x02\x12\x34"
        )  # Positive response
    ...
    def test_read_dir_extra_bytes_response(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x05\x02\xab\xcd\x00\x00\x02\x12\x34\xaa"
        )  # Positive response
    ...
    def test_read_dir_extra_bytes_response_zero_padding_ok(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x78\x05\x02\xab\xcd\x00\x00\x02\x12\x34\x00"
        )  # Positive response
    ...
    def test_wrong_service_reply(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put(
            b"\x71\x01\x02\xab\xcd\x00"
        )  # Reply service ID of routine control
    ...
    def _test_default_filesize_width(self):
        self.udsclient.request_file_transfer(
            moop=1,
            path="my_file.txt",
            filesize=Filesize(uncompressed=0x222, compressed=0x111),
        )
    ...
    def test_filesize_no_compressed_size(self):
        ...
        self.assertEqual(
            ...
            b"\x38\x01\x00\x0b"
            + "my_file.txt".encode("ascii")
            + b"\x00\x04\x00\x00\x02\x22\x00\x00\x02\x22",
        )
        self.conn.fromuserqueue.put(b"\x78\x01\x02\xab\xcd\x00")  # Positive response

    def _test_filesize_no_compressed_size(self):
        self.udsclient.request_file_transfer(
            moop=1, path="my_file.txt", filesize=Filesize(uncompressed=0x222, width=4)
        )
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.request_file_transfer(
                moop=1, path="a" * (2**16), dfi=good_dfi, filesize=good_filesize
            )

        with self.assertRaises(ValueError):
            self.udsclient.request_file_transfer(
                moop=1, path="", dfi=good_dfi, filesize=good_filesize
            )

        with self.assertRaises(ValueError):
            self.udsclient.request_file_transfer(
                moop=1, path="hello.txt", dfi=1, filesize=good_filesize
            )

        with self.assertRaises(ValueError):
            self.udsclient.request_file_transfer(
                moop=1, path="hello.txt", dfi=good_dfi, filesize="asd"
            )

        for moop in [2, 4, 5]:  # Delete File, Read File, Read Dir
            with self.assertRaises(ValueError):
                self.udsclient.request_file_transfer(
                    moop=moop, path="hello.txt", dfi=good_dfi, filesize=good_filesize
                )  # unexpected filesize

        for moop in [1, 3]:  # Add File, Replace File
            with self.assertRaises(ValueError):
                self.udsclient.request_file_transfer(
                    moop=moop, path="hello.txt", dfi=good_dfi, filesize=None
                )  # missing filesize
