...
class TestWriteMemoryByAddress(ClientServerTest):
    ...
    def _test_4byte_block(self):
        memloc = MemoryLocation(
            address=0x1234, memorysize=4, address_format=16, memorysize_format=8
        )
        response = self.udsclient.write_memory_by_address(memloc, b"\x66\x77\x88\x99")

        self.assertEqual(
            response.service_data.alfid_echo, memloc.alfid.get_byte_as_int()
        )
    ...
    def _test_config_format(self):
        ...
        response = self.udsclient.write_memory_by_address(
            MemoryLocation(address=0x1234, memorysize=4), b"\x66\x77\x88\x99"
        )
        alfid = AddressAndLengthFormatIdentifier(
            address_format=self.udsclient.config["server_address_format"],
            memorysize_format=self.udsclient.config["server_memorysize_format"],
        )
