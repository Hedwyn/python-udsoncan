...
class TestDynamicallyDefineDataIdentifier(ClientServerTest):
    ...
    def _test_define_by_did_success(self):
        response = self.udsclient.dynamically_define_did(
            0xF201, DynamicDidDefinition(source_did=0x1234, position=1, memorysize=2)
        )
    ...
    def test_define_by_did_composite_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x2c\x01\xf2\x02\x12\x34\x01\x02\x56\x78\x01\x04\x12\x34\x05\x02"
        )
    ...
    def _test_define_by_memloc_success(self):
        memloc = MemoryLocation(
            address=0x5678, memorysize=0x45, address_format=16, memorysize_format=8
        )
    ...
    def test_define_by_memloc_composite_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x2c\x02\xf3\x02\x12\x11\x22\x02\x33\x44\x04\x55\x66\x08"
        )
    ...
    def _test_define_by_memloc_composite_success(self):
        ...
        diddef.add(
            MemoryLocation(
                address=0x1122, memorysize=2, address_format=16, memorysize_format=8
            )
        )
        diddef.add(
            MemoryLocation(
                address=0x3344, memorysize=4, address_format=16, memorysize_format=8
            )
        )
        diddef.add(
            MemoryLocation(
                address=0x5566, memorysize=8, address_format=16, memorysize_format=8
            )
        )
    ...
    def test_define_by_memloc_composite_success_default_format(self):
        ...
        self.assertEqual(
            request,
            b"\x2c\x02\xf3\x02\x24\x00\x00\x11\x22\x00\x02\x00\x00\x33\x44\x00\x04\x00\x00\x55\x66\x00\x08",
        )
        self.conn.fromuserqueue.put(b"\x6c\x02\xf3\x02")  # Positive response
    ...
    def _test_inconsistent_composite_memloc(self):
        ...
        diddef.add(
            MemoryLocation(
                address=0x11, memorysize=2, address_format=8, memorysize_format=8
            )
        )
        diddef.add(
            MemoryLocation(
        ...
        diddef.add(
            MemoryLocation(
                address=0x3344, memorysize=4, address_format=16, memorysize_format=16
            )
        )
        with self.assertRaises(ValueError):
            self.udsclient.dynamically_define_did(0xF300, diddef)
        ...
        diddef.add(
            MemoryLocation(
                address=0x3344, memorysize=4, address_format=32, memorysize_format=8
            )
        )
        with self.assertRaises(ValueError):
            self.udsclient.dynamically_define_did(0xF300, diddef)
    ...
    def _test_clear_all_dynamic_did_negative_response(self):
        ...
        self.assertTrue(
            issubclass(response.service, services.DynamicallyDefineDataIdentifier)
        )
