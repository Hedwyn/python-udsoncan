...
class TestAuthentication(ClientServerTest):
    ...
    def _test_deauthentication_success(self) -> None:
        ...
        self.assertEqual(
            response.service_data.authentication_task_echo,
            Authentication.AuthenticationTask.deAuthenticate,
        )
        self.assertEqual(
            response.service_data.return_value,
    ...
    def _test_deauthentication_success_spr(self) -> None:
        with self.udsclient.suppress_positive_response:
            response: Authentication.InterpretedResponse = (
    ...
    def test_verifyCertificateUnidirectional_success(self) -> None:
        ...
        self.conn.fromuserqueue.put(
            b"\x69\x01\x11\x00\x40\xaa" + bytes(62) + b"\x44\x00\x00"
        )
    ...
    def _test_verifyCertificateUnidirectional_success(self) -> None:
        ...
        self.assertEqual(
            response.service_data.challenge_server, b"\xaa" + bytes(62) + b"\x44"
        )
    ...
    def _test_verifyCertificateBidirectional_success(self) -> None:
        response: Authentication.InterpretedResponse = (
            self.udsclient.verify_certificate_bidirectional(
        ...
        self.assertEqual(
            response.service_data.challenge_server, b"\xee" + bytes(13) + b"\xff"
        )
        self.assertEqual(response.service_data.certificate_server, bytes(1989))
        self.assertEqual(
            response.service_data.proof_of_ownership_server, b"my precious"
        )
    ...
    def test_proofOfOwnership_success(self) -> None:
        ...
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x29\x03\x01\x50\x7f" + bytes(334) + b"\xb7\x00\x00"
        )
    ...
    def _test_proofOfOwnership_success(self) -> None:
        # ISO 14229-1:2020 Table 91 / 92
        response: Authentication.InterpretedResponse = (
            self.udsclient.proof_of_ownership(
    ...
    def test_transmitCertificate_success(self) -> None:
        ...
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x29\x04\x24\x09\x01\xf4\x31" + bytes(498) + b"\xac"
        )
    ...
    def _test_transmitCertificate_success(self) -> None:
        ...
        # (the table missing "certificate evaluation id" - it's also not fixed in ISO 14229-1:2020 Amendment 1)
        response: Authentication.InterpretedResponse = (
            self.udsclient.transmit_certificate(
    ...
    def test_requestChallengeForAuthentication(self) -> None:
        ...
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(
            request, b"\x29\x05\x00\x06" + bytes(9) + b"\x0a" + bytes(4) + b"\x00"
        )
        self.conn.fromuserqueue.put(
            b"\x69\x05\x00\x06"
            + bytes(9)
    ...
    def _test_requestChallengeForAuthentication(self) -> None:
        ...
        self.assertEqual(
            response.service_data.algorithm_indicator,
            b"\x06" + bytes(9) + b"\x0a" + bytes(4) + b"\x00",
        )
        self.assertEqual(
            response.service_data.challenge_server, b"\xaa" + bytes(62) + b"\x44"
        )
    ...
    def test_verifyProofOfOwnershipUnidirectional(self) -> None:
        ...
        self.assertEqual(
            ...
            + bytes(4)
            + b"\x00\x01\x50"
            + bytes.fromhex("7F2182014B7F4E44")
            + bytes(67)
            + bytes.fromhex("5F37820100")
            + bytes(256)
            + b"\x00\x20\xaa"
            + bytes(30)
            + b"\x44\x00\x00",
        ...
        self.conn.fromuserqueue.put(
            b"\x69\x06\x12\x06" + bytes(9) + b"\x0a" + bytes(4) + b"\x00\x00\x00"
        )

    def _test_verifyProofOfOwnershipUnidirectional(self) -> None:
        # ISO 14229-1:2020 Table 105 / 106
        response: Authentication.InterpretedResponse = (
            self.udsclient.verify_proof_of_ownership_unidirectional(
                algorithm_indicator=b"\x06" + bytes(9) + b"\x0a" + bytes(4) + b"\x00",
                proof_of_ownership_client=bytes.fromhex("7F2182014B7F4E44")
                + bytes(67)
                + bytes.fromhex("5F37820100")
                + bytes(256),
                challenge_client=b"\xaa" + bytes(30) + b"\x44",
    ...
    def _test_verifyProofOfOwnershipBidirectional(self) -> None:
        # ISO 14229-1:2020 Table 105 / 106
        response: Authentication.InterpretedResponse = (
            self.udsclient.verify_proof_of_ownership_bidirectional(
                algorithm_indicator=b"\x06" + bytes(9) + b"\x0a" + bytes(4) + b"\x00",
                proof_of_ownership_client=bytes.fromhex("7F2182014B7F4E44")
                + bytes(67)
                + bytes.fromhex("5F37820100")
    ...
    def _test_verifyCertificateUnidirectional_negative(self) -> None:
        ...
        self.assertEqual(
            response.code, Response.Code.CertificateVerificationFailed_InvalidTimePeriod
        )
    ...
    def test_invalid_response(self) -> None:
        ...
        self.wait_request_and_respond(
            b"\x69\x01\x10\x00"
        )  # #4 only one byte length field
        self.wait_request_and_respond(
            b"\x69\x01\x10\x00\x02\x01"
        )  # #5 data smaller than length field
        self.wait_request_and_respond(b"\x69\x05\x10")  # #6 no algorithIndicator
        self.wait_request_and_respond(
            b"\x69\x05\x10\x00\x00\x00\x00"
        )  # #7 small algorithIndicator
    ...
    def _test_bad_param(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.udsclient.request_challenge_for_authentication(
                communication_configuration=0,
                algorithm_indicator=b"\x06" + bytes(9) + b"\x0a" + bytes(4),
            )

        with self.assertRaises(ValueError):
            self.udsclient.request_challenge_for_authentication(
                communication_configuration=0,
                algorithm_indicator=b"\x06"
                + bytes(9)
                + b"\x0a"
                + bytes(4)
                + b"\x00\x00",
        ...
        with self.assertRaises(ValueError):
            self.udsclient.request_challenge_for_authentication(
                communication_configuration=0, algorithm_indicator=[0] * 16
            )
