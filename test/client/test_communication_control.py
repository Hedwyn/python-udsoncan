...
class TestCommunicationControl(ClientServerTest):
    ...
    def _test_comcontrol_enable_node(self):
        ...
        com_type = CommunicationType(
            subnet=CommunicationType.Subnet.node, normal_msg=True
        )
        ...
        response = self.udsclient.communication_control(
            control_type=control_type, communication_type=com_type.get_byte()
        )
        self.assertTrue(response.positive)
        self.assertEqual(response.service_data.control_type_echo, control_type)

        response = self.udsclient.communication_control(
            control_type=control_type, communication_type=com_type.get_byte_as_int()
        )
        self.assertTrue(response.positive)
        self.assertEqual(response.service_data.control_type_echo, control_type)
    ...
    def _test_comcontrol_enable_node_with_enhanced_addr_info(self):
        ...
        response = self.udsclient.communication_control(
            control_type=control_type, communication_type=com_type, node_id=0x1234
        )
        self.assertTrue(response.positive)
        self.assertEqual(response.service_data.control_type_echo, control_type)
    ...
    def _test_set_params_invalidservice_exception(self):
        with self.assertRaises(InvalidResponseException) as handle:
            control_type = services.CommunicationControl.ControlType.disableRxAndTx
            com_type = CommunicationType(
                subnet=5, normal_msg=True, network_management_msg=True
            )
            self.udsclient.communication_control(
                control_type=control_type, communication_type=com_type
            )
    ...
    def _test_bad_param(self):
        valid_com_type = CommunicationType(
            subnet=3, normal_msg=True, network_management_msg=True
        )
        with self.assertRaises(ValueError):
            self.udsclient.communication_control(
                control_type="x", communication_type=valid_com_type
            )

        with self.assertRaises(ValueError):
            self.udsclient.communication_control(
                control_type=0x80, communication_type=valid_com_type
            )

        with self.assertRaises(ValueError):
            self.udsclient.communication_control(
                control_type=-1, communication_type=valid_com_type
            )
