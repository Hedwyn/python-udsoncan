...
class TestIOControl(ClientServerTest):
    ...
    def _test_io_control_single_reset(self):
        response = self.udsclient.io_control(
            control_param=1, did=0x132
        )  # Reset to default
        self.assertEqual(response.service_data.control_param_echo, 1)
        self.assertEqual(response.service_data.did_echo, 0x132)
        self.assertEqual(
            response.service_data.decoded_data, 0x4A
        )  # 0x4B-1 as defined by codec decode method

    def test_io_control_single_reset_default_did(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, b"\x2f\x01\x32\x01")
        self.conn.fromuserqueue.put(b"\x6f\x01\x32\x01\x4b")  # Positive response

    def _test_io_control_single_reset_default_did(self):
        self.udsclient.config["input_output"] = {"default": StubbedCodec}
    ...
    def test_io_control_variable_size_codec(self):
        ...
        self.conn.fromuserqueue.put(
            b"\x6f\x01\x11\x01\x99\x88\x77"
        )  # Positive response

    def _test_io_control_variable_size_codec(self):
        response = self.udsclient.io_control(
            control_param=1, did=0x111
        )  # Did 111 read all the data.
        self.assertEqual(response.service_data.control_param_echo, 1)
        self.assertEqual(response.service_data.did_echo, 0x111)
        self.assertEqual(
            response.service_data.decoded_data, b"\x99\x88\x77"
        )  # Raw data
    ...
    def _test_io_control_no_control_param(self):
        response = self.udsclient.io_control(
            did=0x132, values=[0x77]
        )  # No control_param, skip directly to data
    ...
    def test_io_control_with_repsonse_record(self):
        ...
        self.conn.fromuserqueue.put(
            b"\x6f\x04\x56\x03\x33\x03\x44\x04"
        )  # Positive response with 0x333 and 0x444 as response data

    def _test_io_control_with_repsonse_record(self):
        response = self.udsclient.io_control(
            control_param=3, did=0x456, values=IOValues(0x111, 0x222)
    ...
    def _test_io_control_with_repsonse_record_zero_padding_not_tolerated_exception(
        self,
    ):
        self.udsclient.config["tolerate_zero_padding"] = False
        for i in range(3):
            with self.assertRaises(InvalidResponseException):
                self.udsclient.io_control(
                    control_param=3, did=0x456, values=IOValues(0x111, 0x222)
                )

    def test_io_control_with_repsonse_record_zero_padding_not_tolerated_no_exception(
        self,
    ):
        data = b"\x6f\x04\x56\x03\x33\x03\x44\x04"
        for i in range(3):
    ...
    def _test_io_control_with_repsonse_record_zero_padding_not_tolerated_no_exception(
        self,
    ):
    ...
    def _test_io_control_composite_did_dict(self):
        ...
        response = self.udsclient.io_control(
            control_param=3, did=0x155, values=values, masks=["IAC_pintle", "pedalA"]
        )  # Short Term Adjustment
        expected_values = {
            "IAC_pintle": 0x07,
            "rpm": 0x2EE,
    ...
    def _test_io_control_non_existent_mask_error(self):
        ...
        with self.assertRaises(ConfigError):
            self.udsclient.io_control(
                control_param=3, did=0x155, values=values, masks=["xxxx"]
            )  # mask xxxx does not exist in config
    ...
    def _test_io_control_mask_all_set1(self):
        values = [0x07, 0x1234, 0x4, 0x5, 0x99]
        self.udsclient.io_control(
            control_param=3, did=0x155, values=values, masks=True
        )  # Short Term Adjustment
    ...
    def _test_io_control_mask_all_set0(self):
        values = [0x07, 0x1234, 0x4, 0x5, 0x99]
        self.udsclient.io_control(
            control_param=3, did=0x155, values=values, masks=False
        )  # Short Term Adjustment
    ...
    def _test_io_control_no_mask(self):
        values = [0x07, 0x1234, 0x4, 0x5, 0x99]
        self.udsclient.io_control(
            control_param=3, did=0x155, values=values
        )  # Short Term Adjustment
    ...
    def _test_io_control_bad_mask_size(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.config["input_output"][0x155]["mask_size"] = (
                None  # Bad value
            )
            self.udsclient.io_control(control_param=3, did=0x155, values=values)
    ...
    def _test_io_control_bad_mask(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.config["input_output"][0x155]["mask"][
                "pedalA"
            ] = -1  # Bad value
            self.udsclient.io_control(control_param=3, did=0x155, values=values)

        with self.assertRaises(ValueError):
            self.udsclient.config["input_output"][0x155]["mask"]["pedalA"] = (
                0x10000  # Bigger than max_size (2)
            )
            self.udsclient.io_control(control_param=3, did=0x155, values=values)
